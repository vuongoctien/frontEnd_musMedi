...
class ExcelAdmin extends Component {
    constructor(props) {
        super(props)
        this.state = {
            listClinic: [],
    ...
    xuatDuLieu = async () => {
        let res2 = await getOrderByClinic({ clinicID: this.state.csytDuocChon.value })
        if (res2 && res2.errCode === 0) {
            let data = res2.data
                    ...
                    <h4>Chức năng truy xuất dữ liệu của musMedi vẫn chưa hoàn thiện. Để truy xuất dữ liệu, vui lòng&nbsp;
                        <i className='xuatraexcel' onClick={async () => {
                            ...
                            await CommonUtils.exportExcel(data,
                                ...
                                `${this.stringDate(this.state.ngayBatDau)} _ ${this.stringDate(this.state.ngayKetThuc)} _ ${this.state.csytDuocChon.name}`)
                        }}>xuất ra Excel</i>
                    </h4>
                </div>
                <div className=''>
...
export default connect(mapStateToProps, mapDispatchToProps)(ExcelAdmin);
